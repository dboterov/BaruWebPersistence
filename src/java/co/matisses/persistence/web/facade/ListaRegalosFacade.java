...
public class ListaRegalosFacade extends AbstractFacade<ListaRegalos> {
    ...
    public List<ListaRegalos> obtenerListasBodasProximas() {
        StringBuilder sb = new StringBuilder();

        sb.append("SELECT * ");
        sb.append("FROM LISTA_REGALOS ");
        sb.append("WHERE CAST(GETDATE() + 3 AS date) = fechaEvento");

        try {
            return em.createNativeQuery(sb.toString(), ListaRegalos.class).getResultList();
        } catch (Exception e) {
            CONSOLE.log(Level.SEVERE, "Ocurrio un error obteniendo el listado de proximas bodas. ", e);
            return null;
        }
    }

    public List<ListaRegalos> obtenerListasBodasVencidas() {
        StringBuilder sb = new StringBuilder();
